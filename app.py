...
with col1:
    ...
    with st.container(border=True):
        ...
        with st.form("form_nuevo_evento", clear_on_submit=True):
            ...
            # --- CORRECCIÓN LÓGICA ---
            submitted_add = st.form_submit_button("➕ Agregar Evento", use_container_width=True)
            if submitted_add:
    ...
    if st.session_state.eventos:
        ...
        if evento_a_editar:
            with st.form("form_editar_evento"):
                ...
                # --- CORRECCIÓN LÓGICA ---
                submitted_edit = col_edit.form_submit_button("💾 Guardar", use_container_width=True)
                submitted_delete = col_del.form_submit_button("🗑️ Eliminar", use_container_width=True)
